...
for (let i=0; i < heroes.length; i++) {
    function createCard(){
        ...
        <div class="maininfo">
            <div><span class="universe bold">Вселенная: </span>${heroes[i].universe}</div>
            <div><span class="alterego bold">Альтер эго: </span>${heroes[i].alterego}</div>
            <div><span class="occupation bold">Род деятельности: </span>${heroes[i].occupation}</div>
            <div><span class="friends bold">Друзья: </span>${heroes[i].friends}</div>
            <div><span class="superpowers bold">Суперсилы: </span>${heroes[i].superpowers}</div>
        </div>
